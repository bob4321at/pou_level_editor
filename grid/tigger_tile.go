...
func (tile *TriggerTile) Serialize(chunk_x, chunk_y, tile_x, tile_y int) TriggerTileJson {
	...
	new_trigger_tile = TriggerTileJson{utils.Vec2{X: float64(chunk_x*1024) + float64(tile_x)*32, Y: float64(chunk_y*1024) + float64(tile_y)*32}, signal}

	return new_trigger_tile
}

func (breakable_tile *TriggerTileJson) Deserialize(tile *int) TriggerTile {
				...
				if (chunk_y*32*32) < int(world_cord_y) && (chunk_y*32*32)+(32*32) > int(world_cord_y) {
					...
					if can_add {
						if level.Level_In_Matrix[chunk_y][chunk_x].Tiles[(int(world_cord_y)/32)-(chunk_y*32)][(int(world_cord_x)/32)-(chunk_x*32)] == -5 {
							level.TriggerTile = append(level.TriggerTile, TriggerTile{&level.Level_In_Matrix[chunk_y][chunk_x].Tiles[(int(world_cord_y)/32)-(chunk_y*32)][(int(world_cord_x)/32)-(chunk_x*32)], "0"})
